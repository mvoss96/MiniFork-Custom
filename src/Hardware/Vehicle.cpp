...
void setupVehicle()
{
    ...
    // Initialize the motor lift
    ledcAttach(PIN_MOTOR_LIFT_A, 5000, 8);
    ledcAttach(PIN_MOTOR_LIFT_B, 5000, 8);
...
void setLiftMotorSpeed(int16_t speed)
{
    ...
    dutyCycle = ((1 << 8) - 1) - dutyCycle; // Invert the duty cycle for slow decay mode (active braking)
    ledcWrite(PIN_MOTOR_LIFT_A, speed > 0 ? dutyCycle : 255);
    ledcWrite(PIN_MOTOR_LIFT_B, speed < 0 ? dutyCycle : 255);
}
